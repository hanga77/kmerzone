...
const AnnouncementForm: React.FC<{ announcement?: Announcement | null; onSave: (data: any) => void; onCancel: () => void }> = ({ announcement, onSave, onCancel }) => {
    ...
    return (
        <form onSubmit={e => { e.preventDefault(); onSave(data); }} className="space-y-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
            <input name="title" value={data.title} onChange={handleChange} placeholder="Titre de l'annonce" className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" required />
            <textarea name="content" value={data.content} onChange={handleChange} placeholder="Contenu de l'annonce" className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" required />
            ...
            <select name="target" value={data.target} onChange={handleChange} className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600">
                ...
                <option value="sellers">Vendeurs seulement</option>
            </select>
            <label className="flex items-center gap-2"><input type="checkbox" name="isActive" checked={data.isActive} onChange={handleChange} /> Activer l'annonce</label>
...
const AdForm: React.FC<{ ad?: Advertisement | null, onSave: (data: Omit<Advertisement, 'id'>) => void, onCancel: () => void }> = ({ ad, onSave, onCancel }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="p-4 space-y-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
            <input name="imageUrl" value={data.imageUrl} onChange={handleChange} placeholder="URL de l'image" className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" required />
            <input name="linkUrl" value={data.linkUrl} onChange={handleChange} placeholder="URL du lien" className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" required />
            <label className="flex items-center gap-2"><input type="checkbox" name="isActive" checked={data.isActive} onChange={handleChange} /> Activer la publicité</label>
            <div className="flex justify-end gap-2">
                <button type="button" onClick={onCancel} className="px-4 py-2 bg-gray-200 rounded">Annuler</button>
                <button type="submit" className="px-4 py-2 bg-blue-500 text-white rounded">Sauvegarder</button>
            </div>
    ...
    return (
        <div className="p-4 sm:p-6">
            <div className="flex border-b dark:border-gray-700 mb-6">
                <PanelButton tab="flashSales" label="Ventes Flash" activeTab={subTab} setActiveTab={setSubTab}/>
                <PanelButton tab="ads" label="Publicités" activeTab={subTab} setActiveTab={setSubTab}/>
                <PanelButton tab="announcements" label="Annonces" activeTab={subTab} setActiveTab={setSubTab}/>
            </div>
            {subTab === 'flashSales' && (
                 <div>
                    <button onClick={() => setIsCreatingFlashSale(true)} className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg mb-4 flex items-center gap-2"><PlusIcon className="w-5 h-5"/> Créer une Vente Flash</button>
            ...
            {subTab === 'ads' && (
                <div>
                     {!isAdFormOpen && <button onClick={() => { setEditingAd(null); setIsAdFormOpen(true); }} className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg mb-4 flex items-center gap-2"><PlusIcon className="w-5 h-5"/> Ajouter une Publicité</button>}
                ...
                <div>
                    {!isAnnouncementFormOpen && (
                        <button onClick={() => { setEditingAnnouncement(null); setIsAnnouncementFormOpen(true); }} className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg mb-4 flex items-center gap-2">
                            <PlusIcon className="w-5 h-5"/> Créer une Annonce
                        </button>
                    )}
