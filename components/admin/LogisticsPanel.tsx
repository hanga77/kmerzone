    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold">Personnel - {point.name}</h3>
                <button onClick={onClose}><XIcon className="w-6 h-6"/></button>
            </div>
            
            <div>
                <h4 className="font-bold text-md mb-2">Responsable</h4>
                {manager ? (
            ...
            <div className="mt-4">
                 <h4 className="font-bold text-md mb-2">Personnel</h4>
                {staff.length > 0 ? (
                    ...
                    <p className="text-center text-gray-500 text-sm py-4">Aucun agent assigné à ce dépôt.</p>
                )}
            </div>
        </div>
    </div>
...
const PointForm: React.FC<{ point?: PickupPoint | null, onSave: (data: any) => void, onCancel: () => void }> = ({ point, onSave, onCancel }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg space-y-4">
             <input name="name" value={data.name} onChange={handleChange} placeholder="Nom du point relais" className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" required />
             <div className="grid grid-cols-2 gap-4">
                <input name="city" value={data.city} onChange={handleChange} placeholder="Ville" className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" required />
                <input name="neighborhood" value={data.neighborhood} onChange={handleChange} placeholder="Quartier" className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" required />
             </div>
             <input name="street" value={data.street} onChange={handleChange} placeholder="Rue / Repère" className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" />
                ...
                {allPickupPoints.map(point => (
                    <div key={point.id} className="p-3 bg-gray-100 dark:bg-gray-700/50 rounded-md flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setViewingStaffFor(point)} className="text-blue-500 flex items-center gap-1 text-sm"><UsersIcon className="w-4 h-4"/> Voir personnel</button>
