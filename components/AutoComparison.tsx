...
const AutoComparison: React.FC<AutoComparisonProps> = ({ currentProduct, otherOffers, stores, onProductClick }) => {
  ...
  return (
    <div className="my-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
      <h3 className="font-bold text-lg mb-4 text-gray-800">Comparer les offres pour ce produit</h3>
      <div className="space-y-3">
        {allOffers.sort((a,b) => getPrice(a) - getPrice(b)).map(offer => {
            ...
            return (
              <div key={offer.id} className={`p-3 border rounded-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 transition-all ${isCurrent ? 'bg-kmer-green/10 border-kmer-green' : 'bg-white border-gray-200'}`}>
                ...
                <div className="flex w-full sm:w-auto items-center justify-between sm:justify-end sm:gap-4">
                  <div className="text-left sm:text-right">
                       <p className={`font-bold text-base sm:text-lg ${isBestPrice ? 'text-kmer-red' : 'text-gray-800'}`}>
                          {price.toLocaleString('fr-CM')} FCFA
                       </p>
                       <p className={`text-xs ${offer.stock > 0 ? 'text-gray-500' : 'text-red-500'}`}>
