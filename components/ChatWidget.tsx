...
import type { Chat, Message, User, Product } from '../types';
...
const ChatWidget: React.FC<{allUsers: User[], allProducts: Product[]}> = ({ allUsers, allProducts }) => {
  const { user } = useAuth();
        ...
        <div className={`w-full lg:w-1/3 flex-shrink-0 transition-transform duration-300 ${activeChatId ? 'hidden lg:block' : ''}`}>
         ...
         <div className={`w-full lg:w-2/3 flex-shrink-0 transition-transform duration-300 ${!activeChatId ? 'hidden lg:flex' : 'flex'}`}>
            <MessageThread
                ...
                currentUser={user}
                onSendMessage={(text) => activeChatId && sendMessage(activeChatId, text, allProducts)}
