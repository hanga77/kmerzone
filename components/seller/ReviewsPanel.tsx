            ...
            <div className="space-y-4">
                {allReviews.map(review => (
                    <div key={`${review.productId}-${review.date}`} className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                        ...
                        <div className="flex items-center gap-2"><StarIcon className="w-4 h-4 text-yellow-400"/><span>{review.rating}/5 par {review.author}</span></div>
                        <p className="italic">"{review.comment}"</p>
                        {review.sellerReply ? (
                            <div className="mt-2 p-2 bg-green-100 dark:bg-green-900/50 rounded-md text-sm"><strong>Votre réponse :</strong> {review.sellerReply.text}</div>
                        ) : (
                            replyingTo === `${review.productId}-${review.date}` ? (
                                <div className="mt-2">
                                    ...
                                    <button onClick={() => handleReply(review.productId, review)} className="bg-blue-500 text-white text-xs px-2 py-1 rounded">Envoyer</button>
                                </div>
                            ) : (
                                <button onClick={() => setReplyingTo(`${review.productId}-${review.date}`)} className="text-sm text-blue-500 mt-2">Répondre</button>
                            )
                        )}
                    </div>
                ))}
