...
const OverviewPanel: React.FC<OverviewPanelProps> = ({ store, sellerOrders, sellerNotifications, onNavigateToAnalytics, setActiveTab }) => {
    ...
    return (
        <div className="p-6 space-y-6">
            <h2 className="text-2xl font-bold">{t('sellerDashboard.overview.title', store.name)}</h2>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <StatCard label={t('sellerDashboard.overview.totalRevenue')} value={`${totalRevenue.toLocaleString('fr-CM')} FCFA`} icon={<ChartPieIcon className="w-6 h-6 text-green-500"/>} />
                <StatCard label={t('sellerDashboard.overview.pendingOrders')} value={pendingOrders} icon={<ShoppingBagIcon className="w-6 h-6 text-yellow-500"/>} />
                <StatCard label={t('sellerDashboard.overview.status')} value={<span className="capitalize">{store.premiumStatus.replace('_', ' ')}</span>} icon={<StarIcon className="w-6 h-6 text-blue-500"/>} />
            </div>
            ...
            {store.warnings && store.warnings.length > 0 && (
                 <div className="p-4 bg-red-50 dark:bg-red-900/50 border-l-4 border-red-500 rounded-r-lg">
                    <h3 className="font-bold text-lg text-red-800 dark:text-red-200 flex items-center gap-2"><ExclamationTriangleIcon className="w-6 h-6"/> {t('sellerDashboard.overview.warnings')}</h3>
            ...
            <div className="p-4 bg-blue-50 dark:bg-blue-900/50 rounded-lg">
                <h3 className="font-bold text-lg text-blue-800 dark:text-blue-200 flex items-center gap-2"><BellIcon className="w-6 h-6"/> {t('sellerDashboard.overview.notifications')}</h3>
                <ul className="mt-2 space-y-1 text-sm">
                    {sellerNotifications.slice(0, 3).map(n => (
                        <li key={n.id} className="text-blue-700 dark:text-blue-300">{n.message}</li>
                    ))}
                    {sellerNotifications.length === 0 && <p className="text-gray-500">{t('sellerDashboard.overview.noNotifications')}</p>}
                </ul>
            </div>
        </div>
    );
